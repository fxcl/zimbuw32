/*
 * Generated from Zimbu file examples/ws_zwt_demo.zu
 */
#include <string.h>
#include <errno.h>
#include <stdlib.h>
...
void YGKs(Y1QG *t, Tc *Apath, Tcb *Ahandler); /* MHTTPModule__CServer.addWebSocket */
...
Yih7 *Yd_c(Yih7 *t, Tc *Apath, Tcb *Ahandler); /* MHTTPModule__CWebSocketServlet.NEW */
To ToYih7[3];
char *StrYih7[];
...
Tc *Y1Ch(YAdf *t); /* MHTTPModule__CWebSocketConnection.recv */
Ts Ymup(YAdf *t, Tc *Atext); /* MHTTPModule__CWebSocketConnection.send */
...
/* ARGModule done */
/* including ws_zwt_page declarations */
Zs11 Yiqh = {&Zstatic, "\011WsZwtPage\000"};
Tt WsZwtPage__T = {4, ((Tc*)&Yiqh), 0};
/* ws_zwt_page done */
Zs3 YXa = {&Zstatic, "\001p\000"};
Zs6 Yb7c = {&Zstatic, "\004port\000"};
Zs38 YV9w = {&Zstatic, "\044Port for the HTTP + WebSocket server\000"};
YJQu *Vport = 0;
void YxSO(YAdf *Aconn); /* wsEcho */
Zs12 YyGk = {&Zstatic, "\012index.html\000"};
Zs23 Ya_P = {&Zstatic, "\025Zimbu ZWT + WebSocket\000"};
Zs13 YLnk = {&Zstatic, "\013/index.html\000"};
Zs5 Y2_6 = {&Zstatic, "\003/ws\000"};
Zs37 YTmW = {&Zstatic, "\043ws_zwt_demo: open http://localhost:\000"};
/*** callbacks for methods invoked directly ***/
void ZcbYxSO(Tcb *cb, YAdf *arg0);
Tcb ZCB_YxSO = {0, ZcbYxSO, -1};
...
Tc YtPC[]="Config.zu";
Tc YfYs[]="examples/ws_zwt_demo.zu";
...
Tc YOtS[]="HTTP.Server.addWebSocket()";
Tcpos ZcTbl61980[]={
{445,11},
{1,7},
};
...
Tc Yt0R[]="HTTP.WebSocketConnection.recv()";
Tcpos ZcTbl95984[]={
{1073,11},
{1,20},
{2,17},
...
Tc YOHX[]="HTTP.WebSocketConnection.send()";
Tcpos ZcTbl46178[]={
{1122,11},
{1,20},
{2,4},
...
Tc Yqqs[]="HTTP.WebSocketServlet.NEW()";
Tcpos ZcTbl37411[]={
{1038,5},
{1,13},
{2,16},
...
Zfo YGKsfo[4] = {{0,(Tt*)&Y1QG__T},{0,(Tt*)&string__T},{0,(Tt*)&cb__T},{0,0}};
void YGKs(Y1QG *t, Tc *Apath, Tcb *Ahandler) {
 Tn *e = ZgetEnv();
 Zsf sf;
 static int sfF = 0;
 if (!sfF) {
  sfF = 1;
  YGKsfo[0].off = (void*)&sf - (void*)&t;
  YGKsfo[1].off = (void*)&sf - (void*)&Apath;
  YGKsfo[2].off = (void*)&sf - (void*)&Ahandler;
 }
 sf.frof = YGKsfo;
 sf.prev = e->topFrame;
 sf.pos = 0x40000000;
 ...
 sf.pos=1079939825;
 ZLap((Tl*)t->VwebSockets, (Tz)(void*)Yd_c(NULL, Apath, Ahandler));
 e->topFrame = sf.prev;
 return;
}
...
Zfo Yd_cfo[4] = {{0,(Tt*)&Yih7__T},{0,(Tt*)&string__T},{0,(Tt*)&cb__T},{0,0}};
Yih7 *Yd_c(Yih7 *t, Tc *Apath, Tcb *Ahandler) {
 Tn *e = ZgetEnv();
 Zsf sf;
 static int sfF = 0;
 if (!sfF) {
  sfF = 1;
  Yd_cfo[0].off = (void*)&sf - (void*)&t;
  Yd_cfo[1].off = (void*)&sf - (void*)&Apath;
  Yd_cfo[2].off = (void*)&sf - (void*)&Ahandler;
 }
 sf.frof = Yd_cfo;
 sf.prev = e->topFrame;
 sf.pos = 0x40000000;
 e->topFrame = &sf;
 if (e->wantGC) ZthreadGC();
 if (t == NULL) {sf.pos=1077482924; t = Za(sizeof(Yih7));}
 sf.pos=3741101;
 t->Vpath = Apath;
 sf.pos=3741102;
 t->Vhandler = Ahandler;
...
Zfo Y1Chfo[3] = {{0,(Tt*)&YAdf__T},{0,(Tt*)&string__T},{0,0}};
Tc *Y1Ch(YAdf *t) {
 Ti Vgot;
 Tn *e = ZgetEnv();
 Zsf sf;
 ...
 if (!sfF) {
  sfF = 1;
  Y1Chfo[0].off = (void*)&sf - (void*)&t;
  Y1Chfo[1].off = (void*)&sf - (void*)&Vmsg;
 }
 sf.frof = Y1Chfo;
 sf.prev = e->topFrame;
 sf.pos = 0x40000000;
 ...
 sf.pos=1083340227;
      {
        unsigned char hdr[2];
        int n = 0;
        while (n < 2) { int r = sckt_read(t->Vfd, (char *)hdr + n, 2 - n); if (r <= 0) goto ws_done; n += r; }
        int opcode = hdr[0] & 0x0F;
        int masked = hdr[1] & 0x80;
        unsigned long long plen = hdr[1] & 0x7F;
        if (plen == 126)
        {
          unsigned char ext[2]; n = 0;
          while (n < 2) { int r = sckt_read(t->Vfd, (char *)ext + n, 2 - n); if (r <= 0) goto ws_done; n += r; }
          plen = ((unsigned long long)ext[0] << 8) | ext[1];
        }
        else if (plen == 127)
        {
          unsigned char ext[8]; n = 0;
          while (n < 8) { int r = sckt_read(t->Vfd, (char *)ext + n, 8 - n); if (r <= 0) goto ws_done; n += r; }
          plen = 0; int ei; for (ei = 0; ei < 8; ei++) plen = (plen << 8) | ext[ei];
        }
        if (plen > 65535) goto ws_done;
        unsigned char mask[4];
        if (masked)
        {
          n = 0;
          while (n < 4) { int r = sckt_read(t->Vfd, (char *)mask + n, 4 - n); if (r <= 0) goto ws_done; n += r; }
        }
        unsigned char payload[65536];
        unsigned long long g = 0;
        while (g < plen) { int r = sckt_read(t->Vfd, (char *)payload + g, (int)(plen - g)); if (r <= 0) goto ws_done; g += r; }
        if (masked) { unsigned long long j; for (j = 0; j < plen; j++) payload[j] ^= mask[j % 4]; }
        payload[plen] = 0;
        if (opcode == 0x8) goto ws_done;   /* close frame */
        Vmsg = Zstr((char *)payload);
        Vgot = 1;
        ws_done: ;
      }
 if (e->wantGC) ZthreadGC();
...
Zfo Ymupfo[3] = {{0,(Tt*)&YAdf__T},{0,(Tt*)&string__T},{0,0}};
Ts Ymup(YAdf *t, Tc *Atext) {
 Ts Vret;
 Tn *e = ZgetEnv();
 Zsf sf;
 Ts r = 0;
 ...
 if (!sfF) {
  sfF = 1;
  Ymupfo[0].off = (void*)&sf - (void*)&t;
  Ymupfo[1].off = (void*)&sf - (void*)&Atext;
 }
 sf.frof = Ymupfo;
 sf.prev = e->topFrame;
 sf.pos = 0x40000000;
 ...
 sf.pos=1078359626;
      {
        ...
        unsigned char out[65540];
        out[0] = 0x81;   /* FIN | text */
        int ol;
        if (len < 126) { out[1] = (unsigned char)len; ol = 2; }
        else { out[1] = 126; out[2] = (unsigned char)(len >> 8); out[3] = (unsigned char)len; ol = 4; }
        Ti i;
        for (i = 0; i < len; i++) out[ol + i] = (unsigned char)p[i];
        ol += (int)len;
        if (sckt_write(t->Vfd, (char *)out, (size_t)ol) == (size_t)ol) Vret = 1;
      }
 if (e->wantGC) ZthreadGC();
 sf.pos=1078359627;
 r = Vret;
...
/* ARGModule done */
Zfo YxSOfo[3] = {{0,(Tt*)&YAdf__T},{0,(Tt*)&string__T},{0,0}};
void YxSO(YAdf *Aconn) {
 Tn *e = ZgetEnv();
 Zsf sf;
 Tc *Vmsg = NULL;
 static int sfF = 0;
 ...
 if (!sfF) {
  sfF = 1;
  YxSOfo[0].off = (void*)&sf - (void*)&Aconn;
  YxSOfo[1].off = (void*)&sf - (void*)&Vmsg;
 }
 sf.frof = YxSOfo;
 sf.prev = e->topFrame;
 sf.pos = 0x40000000;
...
int ZglobInit(int round) {
 ...
 if (round == 1) {
   sf.pos=1073741824;
   Vport = Y4qx(NULL, ((Tc*)&YXa), ((Tc*)&Yb7c), 8096, ((Tc*)&YV9w));
 }
...
int Fmain(void) {
 ...
 sf.pos=1074268826;
 Y1DX(Vserver, ((Tc*)&YyGk), ((Tc*)&Ya_P), &WsZwtPage__T);
 sf.pos=1074268827;
 Yv7Q(Vserver, ((Tc*)&YV), ((Tc*)&YLnk));
 sf.pos=1074268828;
 YGKs(Vserver, ((Tc*)&Y2_6), (&ZCB_YxSO));
 sf.pos=1074268829;
 Y1QG__YTgs__YSuw(Vserver);
 sf.pos=1074268830;
 Yl0k(ZcS3(((Tc*)&YTmW), Zint2string(Yl8r(Vport)), ((Tc*)&YV)));
 sf.pos=1074268831;
 YQj8(Vserver, ((Tc*)&YyGk));
 sf.pos=1074268832;
 YqJz(Vserver);
 sf.pos=1074268833;
 r = 0;
